...
//    {
//    CAN_ResetFilter(rbCAN->canHandler[i]);
//    }
    ...
    while(rbCAN->isWorking == true){
        ...
        for(int i=0; i<rbCAN->chNum; i++){            
            while(LINUX_CAN_Read_Timeout(rbCAN->canHandler[i], &m, 0) == 0){
                ...
                {
                    unsigned int temp_id = (unsigned int)(m.Msg.ID);
                    unsigned int temp_len = (unsigned int)(m.Msg.LEN);
                    //std::cout<<"NOT REGISTERD ID "<<temp_id<<" LEN "<<temp_len<<" ch "<<i<<std::endl;
                    if(i==0)
                    {
                    DWORD tt = CAN_Status(rbCAN->canHandler[i]);
                    TPSTATUS tt2;
                    memcpy(&tt2,&tt,sizeof(TPSTATUS));
                    //std::cout<<"can_status "<<std::hex <<tt2.wErrorFlag<<"  "<<tt2.nLastError<<std::endl;
                    }
                }
            }
        }
...
RBCAN::RBCAN(int _ChNum){
    ...
    for(int i=0; i<MAX_SEARCH_CHANNEL; i++){
        char filePath[50];
//        if(i == 0)
//            sprintf(filePath, "/dev/pcanusb%d", i+32);
//        else
//            sprintf(filePath, "/dev/pcanusbfd%d", i+32);

        sprintf(filePath, "/dev/pcan%d", i+32);
        canHandler[oknum] = NULL;
        canHandler[oknum] = LINUX_CAN_Open(filePath, O_RDWR);

        if(canHandler[oknum] != NULL){
            if(CAN_Init(canHandler[oknum], CAN_BAUD_1M, CAN_INIT_TYPE_EX) !=  CAN_ERR_OK){
...
void RBCAN::RBResetCAN(){
    ...
    {
        char filePath[30];
//        sprintf(filePath, "/dev/pcan%d", i);
//        sprintf(filePath, "/dev/pcanusb%d", i);
        sprintf(filePath, "/dev/pcan%d", i+32);
//        sprintf(filePath, "/dev/pcan%d", i);
//        sprintf(filePath, "/dev/pcanusbfd%d", i);
//        sprintf(filePath, "/dev/pcanusbpfd%d", i);
        canHandler[oknum] = NULL;
        canHandler[oknum] = LINUX_CAN_Open(filePath, O_RDWR);
...
////        sprintf(filePath, "/dev/pcan%d", i);
////        sprintf(filePath, "/dev/pcanusbfd%d", i+32);
//        sprintf(filePath, "/dev/pcanusbpfd%d", i+32);
//        canHandler[oknum] = NULL;
//        canHandler[oknum] = LINUX_CAN_Open(filePath, O_RDWR);
...
{
    pthread_t thread_CANRead;
    if(generate_rt_thread(thread_CANRead, RBCAN_ReadThread, "CANRead_thread", 1, 95, NULL)) {
        FILE_LOG(logSUCCESS) << "RBCAN: Read thread start = OK";
        return true;
    } else {
...
int RBCAN::RBCAN_WriteDataDirectly(RBCAN_MB _mb)
{
    ...
    m.ID = _mb.id;
    m.LEN = _mb.dlc;
    m.MSGTYPE = MSGTYPE_STANDARD;//MSGTYPE_EXTENDED vs MSGTYPE_STANDARD
...
// --------------------------------------------------------------------------------------------- //
int RBCAN::RBCAN_SeeData(pRBCAN_MB _mb)
{
    unsigned int index = RBCAN_GetMailBoxIndex(_mb->id);
