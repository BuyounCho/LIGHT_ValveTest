...
public:
    ...
    void    SetRefValvePosCurrent(const double ref)	{RefValveCurrent = ref;}
    double  GetRefValvePosCurrent()					{return RefValveCurrent;}
...
public:
    ...
    double  GetJointRefValvePos(const int n)                    {return Joints[n]->GetRefValvePosCurrent();}
    void    SetJointRefValvePos(const int n, const double _ref)	{Joints[n]->SetRefValvePosCurrent(_ref);}
...
//            Shm_ref->VelocityReference[PODONum][mcId][mcCh] = Joints[i]->GetRefLinVelCurrent();
            ...
            Joints[i]->SetRefValvePosCurrent(Shm_sen->ENCODER[mcId][mcCh].CurrentRefValvePos);
            Shm_ref->ValvePosReference[PODONum][mcId][mcCh] = Joints[i]->GetRefValvePosCurrent();
