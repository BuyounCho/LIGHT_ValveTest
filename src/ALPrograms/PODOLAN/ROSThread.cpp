...
void ROSWorker::SendtoROS(){
    for(int i=0; i<NO_OF_JOINTS; i++){
        TXData.JointReference[i] = sharedSEN->ENCODER[MC_GetID(i)][MC_GetCH(i)].CurrentRefAngle;
        TXData.JointEncoder[i] = sharedSEN->ENCODER[MC_GetID(i)][MC_GetCH(i)].CurrentAngle;
    }
