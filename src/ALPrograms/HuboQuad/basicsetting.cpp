...
int RBInitialize(void){
    ...
    char task_thread_nameQP[30];
    char flag_thread_name[30];
    sprintf(task_thread_name, "%s_TASK", __AL_NAME);
    sprintf(task_thread_nameQP, "%s_TASKQP", __AL_NAME);
    ...
    usleep(100*1000);//wait for init
    if(rt_task_create(&rtQPTaskCon, task_thread_nameQP, 0, 90, 0) == 0){
        cpu_set_t aCPU;
        CPU_ZERO(&aCPU);
        CPU_SET(1, &aCPU);//hmm....
        if(rt_task_set_affinity(&rtQPTaskCon, &aCPU) != 0){
            FILE_LOG(logWARNING) << "QPTask real-time thread set affinity CPU failed..";
        }
        if(rt_task_start(&rtQPTaskCon, &RBQPTaskThread, NULL) == 0 ){
            FILE_LOG(logSUCCESS) << "QPTask real-time thread start = OK";
        }else{
            FILE_LOG(logERROR) << "QPTask real-time thread start = FAIL";
            return false;
        }
    }else{
        FILE_LOG(logERROR) <<  "Fail to create QPTask real-time thread";
        return false;
    }
