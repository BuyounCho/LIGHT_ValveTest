...
extern char __AL_NAME[30];// = "AL NAME";
...
inline std::string NowTime(){
    ...
    strftime(buff, 100, "%M:%S", st_time);

    struct timeval tv;
    gettimeofday(&tv, 0);
    char result[50] = {0};
    sprintf(result, "%s\t%s.%03ld", __AL_NAME, buff, (long)tv.tv_usec/1000);
...
std::ostringstream& Log<T>::Get(TLogLevel level)
{
    if(level == logERROR){
        ...
        os << "\033[1;36m";
    }else if(level == logDEBUG1){
        os << "\033[1;35m";
    }else if(level == logDEBUG2){
        os << "\033[1;34m";
    }else if(level == logDEBUG3){
        os << "\033[1;37m";
    }else if(level == logDEBUG4){
        os << "\033[1;30m";
    }
    os << "- " << NowTime();
    os << " " << ToString(level) << ": ";
